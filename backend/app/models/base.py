...
class IntersectionNode(Base):
    ...
    city = Column(String, default="New Delhi", nullable=True)
    full_address = Column(String, nullable=True)
    latitude = Column(Float, default=28.6315, nullable=True)
    longitude = Column(Float, default=77.2167, nullable=True)
...
class VehicleTrack(Base):
    ...
    intersection_id = Column(String, default="junction_1", index=True)
    camera_id = Column(String, default="CAM_01", index=True)
    vehicle_type = Column(String, index=True, nullable=False)
    lane = Column(String, nullable=True)
    entry_lane = Column(String, nullable=True)
    exit_lane = Column(String, nullable=True)
    direction = Column(String, nullable=True)
    confidence = Column(Float, default=0.9)
    bbox = Column(String, nullable=True)
    source = Column(String, default="RECORDED_VIDEO", index=True)
    first_seen = Column(DateTime, default=lambda: datetime.now(timezone.utc), index=True)
    last_seen = Column(DateTime, default=lambda: datetime.now(timezone.utc), onupdate=lambda: datetime.now(timezone.utc), index=True)
...
class VehicleCrossing(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    crossing_id = Column(String, unique=True, index=True, nullable=False)
    ...
    crossed_at = Column(DateTime, default=lambda: datetime.now(timezone.utc), index=True)
...
class TrafficTimeSeries(Base):
    __tablename__ = "traffic_time_series"
    id = Column(Integer, primary_key=True, index=True)
    sensor_id = Column(String, index=True, nullable=False)
    intersection_id = Column(String, default="junction_1", index=True)
    timestamp = Column(DateTime, index=True, nullable=False)
    traffic_speed = Column(Float, nullable=True) # Speed in mph/kmh
    traffic_flow = Column(Float, nullable=True)  # Estimated flow count
    occupancy = Column(Float, nullable=True)     # Occupancy ratio (0.0 to 1.0)
    source = Column(String, default="METR-LA", index=True)
